...
router.post('/users', [auth, isAdmin], async (req, res) => {
    ...
    // Assuming USER_ROLES is available globally or via require
    const validRoles = ['Admin', 'Faculty', 'Student', 'Assistant']; // Or get from USER_ROLES object
...
router.put('/users/:userId', [auth, isAdmin], async (req, res) => {
    ...
    if (!fullName &amp;&amp; !email &amp;&amp; !role &amp;&amp; department === undefined) { // Check if department is undefined to allow setting it to null
    ...
    try {
        ...
        const updatedFields = {
            ...
            department: department !== undefined ? (department || null) : existingUser.department, // Allow setting department to null
        };
...
router.get('/requests/faculty', [auth, isAdmin], async (req, res) => {
    try {
        ...
        const [facultyRequests] = await pool.query(`
            ...
            WHERE b.status = 'pending-admin-approval' AND u.role = 'Faculty' 
...
// --- System Activity Logs (Mock) --- // Endpoint removed as feature was removed
...
// This uses MOCK_TIME_SLOTS if defined in scope, or a hardcoded example if not.
// Ensure MOCK_TIME_SLOTS are consistently available or fetched from DB for actual C++ input.
...
router.post('/algorithms/:algorithmName', [auth, isAdmin], async (req, res) => {
    ...
    try {
        if (algorithmName === 'run-scheduling') { // Graph Coloring - ACTUAL INTEGRATION EXAMPLE
            ...
            // Simulate fetching actual data required by Graph Coloring
            ...
            // CRITICAL CONSTRAINTS FOR C++ GRAPH COLORING:
            ...
            const [labsData] = await pool.query("SELECT id, name, capacity, location, roomNumber FROM labs"); // Example, might need more fields like lab_type
            const [bookingRequestsData] = await pool.query( // Fetching requests that need scheduling
                `SELECT 
                 ...
                 WHERE b.status IN ('pending', 'pending-admin-approval')` // Adjust statuses as per your workflow
            );
            const timeSlots = MOCK_TIME_SLOTS_FROM_CONSTANTS_OR_DB;

            actualInputForCpp = {
                labs: labsData.map(lab => ({ id: lab.id, name: lab.name, capacity: lab.capacity, type: "General" /* Replace with actual lab type */ })),
                labSessionRequests: bookingRequestsData.map(req => ({
                    ...
                    facultyId: `F${req.facultyUserId}`, // Example: ensure consistent faculty ID format for C++
                    studentBatch: req.batchIdentifier || `Batch_For_${req.requestId}`,
                    durationSlots: 1, // Assume 1 slot duration, C++ needs to handle multi-slot if required
                    preferredLabId: req.preferredLabId,
                    requiredLabType: "General", // Example, C++ needs to match this with lab types
                ...
                //   { requestId: 2, courseSection: "CS101_SecA", facultyId: "F10", studentBatch: "Batch_A1", durationSlots: 1, requiredLabType: "Computer", preferredLabId: 1}, // Same section, same faculty
                //   { requestId: 3, courseSection: "PHY202_SecB", facultyId: "F11", studentBatch: "Batch_B1", durationSlots: 2, requiredLabType: "Physics"}, // Different section/faculty
                //   { requestId: 4, courseSection: "CS101_SecB", facultyId: "F12", studentBatch: "Batch_A2", durationSlots: 2, requiredLabType: "Computer"}, // Different section of CS101, potentially different faculty
                // ],
                timeSlots: timeSlots.map(ts => ({id: ts.id, display: ts.displayTime, startTime: ts.startTime, endTime: ts.endTime})),
                facultyAvailability: { // This would be fetched from DB or a faculty settings table
                    "F10": ["ts_0900_1000", "ts_1000_1100", "ts_1100_1200", "ts_1400_1500"],
                    "F11": ["ts_1300_1400", "ts_1400_1500"],
                    "F12": ["ts_0900_1000", "ts_1000_1100", "ts_1600_1700"],
                },
                // Other constraints: student batch schedules (to avoid clashes with their other classes), lab-equipment compatibility, etc.
            };
            ...
            const cppProcess = spawn(cppExecutablePath, []); // Add command line args if any for C++

            let cppOutput = '';
            let cppErrorOutput = '';

            // Handle errors from spawning the process itself
                    ...
                    cppProcess.on('close', async (code) => {
                        ...
                        if (code === 0 &amp;&amp; cppOutput &amp;&amp; cppOutput.trim() !== '') {
                            try {
                                ...
                                //     { requestId: 1, courseSection: "CS101_SecA", assignedLabId: 1, assignedDate: "2024-09-02", assignedTimeSlotId: "ts_1000_1100", facultyId: "F10", studentBatch: "Batch_A1", purpose: "Scheduled: CS101_SecA (L2)"},
                                //     { requestId: 3, courseSection: "PHY202_SecB", assignedLabId: 2, assignedDate: "2024-09-03", assignedTimeSlotId: "ts_1300_1400", facultyId: "F11", studentBatch: "Batch_B1", purpose: "Scheduled: PHY202_SecB (L1)"}
                                //   ],
                                //   unscheduledRequests: [ { requestId: 2, reason: "No suitable slot with faculty F10" } ]
                                // }
                                // NOTE: All lab sessions for a given studentBatch/courseSection must be together in their timetable.
                                // The C++ algorithm must ensure this. If CS101_SecA needs two lab slots, they must be usable by all students in Batch_A1.
                                ...
                                if (actualScheduleFromCpp.newlyScheduledBookings &amp;&amp; Array.isArray(actualScheduleFromCpp.newlyScheduledBookings)) {
                                    for (const session of actualScheduleFromCpp.newlyScheduledBookings) {
                                      // ACTUAL DB: await pool.query(
                                      //     'INSERT INTO bookings (labId, userId, date, timeSlotId, status, purpose, requestedByRole, batchIdentifier) VALUES (?, (SELECT id FROM users WHERE customFacultyId = ?), ?, ?, "booked", ?, ?, ?)',
                                      ...
                                      // );
                                      // ACTUAL DB: await pool.query("UPDATE bookings SET status = 'scheduled_by_system' WHERE id = ?", [session.requestId]);
                                      scheduledCount++;
                                    }
                                }
                                if (actualScheduleFromCpp.unscheduledRequests &amp;&amp; Array.isArray(actualScheduleFromCpp.unscheduledRequests)) {
                                   ...
                                   //    ACTUAL DB: await pool.query("UPDATE bookings SET status = 'scheduling_failed', purpose = CONCAT(IFNULL(purpose,''), ' - Reason: ', ?) WHERE id = ?", [req.reason, req.requestId]);
                                      unscheduledCount++;
                                   }
                                }
                                ...
                                simulatedOutputFromCpp = actualScheduleFromCpp;
                                resolve();
                            } catch (parseError) {
            ...
            actualInputForCpp = {
                ...
                sessionRequests: [ // Sessions that need these resources
            ...
            // SIMULATED C++ OUTPUT for Knapsack
            simulatedOutputFromCpp = {
                status: "success",
                summary: "Optimally allocated scarce resources to 2 sessions.",
                allocatedSessions: [ // Sessions that get the resources
                ...
                unallocatedSessions: [ // Sessions that couldn't get the resources
            ...
            actualInputForCpp = {
                emptyTimeSlots: [ // Derived from current schedule
                ...
                pendingRequests: [ // Sorted by some priority
            ...
            actualInputForCpp = {
                ...
                sourceLocationNodeId: "CS_Dept_Loc", // Example: User is in CS department
                targetLabLocations: [ // Available labs (ID from DB, locationNodeId from campus graph)
            ...
            // SIMULATED C++ OUTPUT for Dijkstra
            simulatedOutputFromCpp = {
                status: "success",
                ...
                shortestPaths: [ 
                    ...
                    { labDbId: 5, name: "Physics Lab Beta (overflow)", distance: 130, path: ["CS_Dept_Loc", "Junction1", "Physics_Dept_Loc", "Lab205_Loc"] } // Example of a longer path
                ],
                recommendation: { labDbId: 1, name: "CS Lab 101" } // The nearest one
            };
            console.log("[Backend] Simulated output from C++ (Dijkstra):", JSON.stringify(simulatedOutputFromCpp, null, 2));
            console.log("[Backend] SIMULATING Usage: This algorithm's output would be used by the booking system to SUGGEST the nearest lab or by an admin for manual assignment. It might not directly update bookings itself.");
        ...
        // Ensure response is sent only if not already handled by cppProcess.on('error') for 'run-scheduling'
        if (!res.headersSent) { 
            res.json({
                success: true,
        ...
        console.error(`[Backend] Error in algorithm trigger simulation for ${algorithmName}:`, error.message, error.stack);
        if (!res.headersSent) {
            res.status(500).json({ 
                success: false, 
                message: `Server error during ${algorithmName} simulation. Details: ${error.message}`,
                algorithm: algorithmName,
                simulatedInputSentToCpp: actualInputForCpp, 
                simulatedOutputReceivedFromCpp: simulatedOutputFromCpp,
